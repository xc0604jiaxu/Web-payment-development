...
import com.xc.common.tools.ReflectionTools;
import com.xc.common.tools.Tools;
...
public class BaseDao<T extends BaseEntity> extends HibernateDaoSupport {
	...
	public T find(String fileName, Object fieldValue) {
		...
		T t = (T) createCriteria(getXSession(), entityClass, criterion).setCacheable(isUseQueryListCache())
				.uniqueResult();
		return t;
	}
	...
	public List<T> findListByIds(List<Long> idList) {
		if (idList != null && idList.size() >= 1) {
			...
			List<T> l = createCriteria(getXSession(), entityClass, criterion).setCacheable(isUseQueryListCache())
					.list();
			return l;
		} else {
			return null;
	...
	public <X> List<X> findList(String hql, Map<String, ?> values, int startPos, int pageSize) {
		Query query = getXSession().createQuery(hql);
		List<X> l = createQuery(getXSession(), hql, values).setFirstResult(startPos).setMaxResults(pageSize)
				.setCacheable(isUseQueryListCache()).list();
		return l;
	}
