...
public class Tools extends JavaTools {
	...
	public static String logHead(HttpServletRequest request) {
		...
		return Tools.logString(System.currentTimeMillis() - stime, request.getMethod(), request.getServerName(),
				getRemoteAddr(request), Tools.removeAll(request.getRequestURI(), "|"),
				Tools.removeAll(request.getQueryString(), "|"), Tools.objectToJsonString(request.getParameterMap()));
	}
