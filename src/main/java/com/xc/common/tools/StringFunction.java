...
public class StringFunction {
                                ...
                                if (tagString.length() > 0) {
                                    if (haveHeadSlash) {
                                        if (wrongContentTag != null && tagString.equals(wrongContentTag))
                                            wrongContent = false;
                                    } else if ("style".equals(tagString) || "script".equals(tagString)
                                            || "xml".equals(tagString)) {
                                        wrongContent = true;
        ...
        if (url != null) {
            ...
            if (npos > 0) {
                ...
                for (String s : pars) {
                    npos2 = s.indexOf('=');
                    if (npos2 > 0)
                        sbuf.append("&").append(s.substring(0, npos2 + 1))
                                .append(encodePar(decodePar(s.substring(npos2 + 1))));
                    else
    ...
    public static String filterSpecialChar(String source, String reStr) {
        ...
        if (stringIsNotNull(source)) {
            Pattern emoji = Pattern.compile("[\ud840\udc00-\ud869\uded6]",
